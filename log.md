# 100 Days Of Code - Log

### Day 1: Jan 4th, Wednesday

**Today's Progress**: I've completed the JS calculator 

**Thoughts**: Getting the funciton to replace an answer value instead of appending took a while. The decimal problems have been solved crudely. 

**Link(s) to work**
1. [JS-Calculator](https://github.com/ClimbinSheep/Projects/commit/b20a11dbab1dffe3b663a369e5adbaab6c815ad6)

----------------------------------------------------------------------
### Day 2: Jan 5th, Thursday

**Today's Progress**: Completed the functionality of the Pomorodo timer

** Thoughts**: Getting to grips with the setInterval function was trickier than expected.

**Link(s) to work**
1. [Pomorodo clock](https://github.com/ClimbinSheep/Projects/commit/8d564471023e31e28352e1c97376300811dbbb33)

----------------------------------------------------------------------
### Day 3: Jan 6th, Friday

**Today's Progress**: created the start of the tic tac toe board

** Thoughts** getting a 3x3 board was quite hard. making sure that the tiles were responsive took a good bit of googling.

**Link(s) to work**
[tic tac toe board](https://github.com/ClimbinSheep/Projects/commit/8d564471023e31e28352e1c97376300811dbbb33)

----------------------------------------------------------------------
### FAIL DAY: Jan 7th, Saturday

**Today's Progress**: Ill 

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### FAIL DAY: Jan 8th, Sunday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day 4: Jan 9th, Monday

**Today's Progress**: moving Random quote machine to github, trying to get it to work 

** Thoughts**: need to work on learning API's better. 

**Link(s) to work**
[random quote machine](https://github.com/ClimbinSheep/Projects/commit/92df302cdd359951843e4709946f13fbc67a68e8)

----------------------------------------------------------------------
### Day 5: Jan 10th, Tuesday

**Today's Progress**: Added API functionality to the random quote generator. Also created a copy with Chuck Norris joke generator.

** Thoughts**: Need to learn how to use api/ getJSON calls better. 

**Link(s) to work**
[random quote machine](https://github.com/ClimbinSheep/Projects/commit/dbed77406b5f0f6d8983c2628d4f0d46b76b542a)
[Chuck Norris joke generator](https://github.com/ClimbinSheep/Projects/commit/721e62e22944233d8f44010a4bff607b99da528f)

----------------------------------------------------------------------
### Day 6: Jan 11th, Wednesday

**Today's Progress**: Added titles to the existing projects and completed functinality of the weather app. finished reading coersion and now reading grammar of YDKJS.

** Thoughts**: getting more used to doing API calls. Reading YDKJS help me learn ternary opperators so used two in the weather app. Very concise bits of code.

**Link(s) to work**
[weather app commit](https://github.com/ClimbinSheep/Projects/commit/5b4fc05674b016ea698c5308fce22d4aec32435f)

----------------------------------------------------------------------
### Day 7: Jan 12th, Thursday

**Today's Progress**: Working on the CSS on a few sites. Leaning about '@media' and responsive design'

** Thoughts**: Need to work more on CSS. Display: flex seems to break 'col-md-6' formatting.

**Link(s) to work**
1.[responsive calculator](https://github.com/ClimbinSheep/Projects/commit/281fcb176309268ca659ff5be330ee8b4b665eeb)
2.[CSS for Pomodoro and ChuckNoris sites](https://github.com/ClimbinSheep/Projects/commit/3413d3fe7e5da05ca7137ff6c21f36a1d0fa8de5)

----------------------------------------------------------------------
### Day 8: Jan 13th, Friday

**Today's Progress**: completed day 1 and 2 on the PureCSS chaalanges

** Thoughts**: really cool way to do things. Trying to get everything good without using clippy or px definitions is hard work.

**Link(s) to work**
1.[PureCSS challenges 1 and 2](https://github.com/ClimbinSheep/Projects/commit/5074ffd0060648c871efe407f5af4d1640a83085)

----------------------------------------------------------------------
### Day 9: Jan 14th, Saturday

**Today's Progress**: installed nodejs on laptop, migrated learnyounode. did p9.js

** Thoughts**: 

**Link(s) to work** 

----------------------------------------------------------------------
### Day 10: Jan 15th, Sunday

**Today's Progress**: Rebuilding my portfolio site using google material design.

** Thoughts**: Material design is very clever.

**Link(s) to work** [New Portfolio site](https://climbinsheep.github.io/Portfolio_site/)

----------------------------------------------------------------------
### Day 11: Jan 16th, Monday

**Today's Progress**: Daily Pure CSS image - beaver

** Thoughts**: natural shapes are hard to do in css.

**Link(s) to work** [PureCSS](https://github.com/ClimbinSheep/Projects/commit/844616d32dc44436e200d0c457da78648002ffbb)

----------------------------------------------------------------------
### Day 12: Jan 17th, Tuesday

**Today's Progress**: Working on portfolio site again. Got it hosted on Github now along with the rest of the projects. Added basic email me contact form but will create a email server to avoid the current solution.

** Thoughts**: good to get portfolio hosted and project websites on there too.

**Link(s) to work**
1. [Portfolio Site](samwCoding.github.io)

----------------------------------------------------------------------
### Day 13: Jan 18th

**Today's Progress**: Learning express.js through nodeschool

** Thoughts**: express.js seems to be pretty good. need to do a lot of practice.

**Link(s) to work**

----------------------------------------------------------------------
### Day 14: Jan 19th, Thursday

**Today's Progress**: Working on pure CSS day 4-6. tried the MongoDB tutorial but it didnt work on the office pc.

** Thoughts**:

**Link(s) to work**:

----------------------------------------------------------------------
### FAIL DAY: Jan 20th, Friday

**Today's Progress**: Wikipedia Viewer Building. Got the structure but struggling with teh requests, Cant use $.getJSON so need to learn about CORS.

** Thoughts**

**Link(s) to work**
----------------------------------------------------------------------
### Day 15: Jan 21st, Saturday

**Today's Progress**: Trying to get a custom form onto my website so that users dont get redirected off of my site.

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### FAIL DAY: Jan 22nd, Sunday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day 16: Jan 23rd, Monday

**Today's Progress**: Only a small amount of work done on my portfolio website

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day 17: Jan 24th, Tuesday

**Today's Progress**: Finally got my wikipedia viewer working. Still cant figure out cross origin PUT requests.

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day 18: Jan 25th, Wednesday

**Today's Progress**: starting with my twitch stream checker. Got the request working, just need to improve the design.

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day 19: Jan 26th, Thursday

**Today's Progress**: Still fighting with CORS PUT request for my profolio site. I will move onto something else tomorrow.

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day 20: Jan 27th, Friday

**Today's Progress**: Working on my pure css images 

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day 21: Jan 28th, Saturday

**Today's Progress**: finishing off my twitch stream checker. added functionality to hide streams that arent active.

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### FAIL DAY: Jan 29th, Sunday

----------------------------------------------------------------------
### Day 22: Jan 30th, Monday

**Today's Progress**: Working on catching up on my pure css, almost fininshed the first week and will be uploading them tomorrow

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day 23: Jan 31st, Tuesday

**Today's Progress**: Working on my Pure CSS of a cartoon character. This is taking a lot longer than I expencted. 

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day 24: Feb 1st, Wednesday

**Today's Progress**: Finally finished my favourite cartoon character 

** Thoughts**

**Link(s) to work** [Top Cat](https://pbs.twimg.com/media/C3kV8DuWIAAGPqA.jpg)

----------------------------------------------------------------------
### Day 25: Feb 2nd, Thursday

**Today's Progress**: Completed 3 more FCC advanced algorithms and started to learn React.

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### FAIL DAY: Feb 3rd, Friday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day 26: Feb 4th, Saturday

**Today's Progress**: Made a simple Clock using pure css and got it functioning with JavaScript

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day 27: Feb 5th, Sunday

**Today's Progress**: Started doing the Pure CSS tape measure

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day 28: Feb 6th, Monday

**Today's Progress**: Started my Shopping List web app using Firebase.

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### FAIL DAY:: Feb 7th, Tuesday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day 29: Feb 8th, Wednesday

**Today's Progress**: Learning React 

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day 30: Feb 9th, Thursday

**Today's Progress**: More learning React. Got into the Chingu Cohort which is great.

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day 31: Feb 10th, Friday

**Today's Progress**: Still trying to get to grips with React

** Thoughts** this is takign wuite a while to get my head around. 

**Link(s) to work**

----------------------------------------------------------------------
### FAIL DAY: Feb 11th, Saturday

----------------------------------------------------------------------
### Day 32: Feb 12th, Sunday

**Today's Progress**: Spent the day cleaning up and updating my Portfolio page and a few of my project sites.

** Thoughts** Applied for jobs yeaterday so want everything to work.

**Link(s) to work**

----------------------------------------------------------------------
### Day 33: Feb 13th, Monday

**Today's Progress**: Building the Simon game for FCC, got the latout, just need to add the JavaScript funcitonality.

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day 34: Feb 14th, Tuesday

**Today's Progress**: almost finished my Simon game functionality. Little things keep it from working well.

** Thoughts** need to stop the user from repeatedly clicking on a button.

**Link(s) to work**

----------------------------------------------------------------------
### Day 35: Feb 15th, Wednesday

**Today's Progress**: Completed the SoloLearn JavaScript course - learnt a few cool things.

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day 36: Feb 16th, Thursday

**Today's Progress**: Learning more React with hysterical-amusement.surge.sh . Done all of the React section but leaving redux until later.

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day 37: Feb 17th, friday

**Today's Progress**: Did a few of the JavaScript 30 projects, the array workout videos are great. 

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day 38: Feb 18th, Saturday

**Today's Progress**: Workign on my Simon Game again

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day 39: Feb 19th, Sunday

**Today's Progress**: almost Finished my simon game, just need to add sound. 

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day 40: Feb 20th, Monday

**Today's Progress**: Working on the AI fro the tic-tac-toe game.

** Thoughts** The recursion is hard to get my head around

**Link(s) to work**

----------------------------------------------------------------------
### Day 41: Feb 21st, Tuesday

**Today's Progress**: modified a JS30 tutorial to make a rainbow whitboard.

** Thoughts**

**Link(s) to work** [rainbow whiteboard](tiny.cc/7kndjy)

----------------------------------------------------------------------
### Day 42: Feb 22nd, Wednesday

**Today's Progress**: Made my first React app over the last 2 days - a markdown previewer

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day 43: Feb 23rd, Thursday

**Today's Progress**: added an upload function to the markdown previewer

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### FAIL DAY: Feb 24th, Friday

----------------------------------------------------------------------
### Day 44: Feb 25th, Saturday

**Today's Progress**: Converting my portfolio ot React

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day 45: Feb 26th, Sunday

**Today's Progress**: started group work on a momentum based chrome extension 

** Thoughts** Lots to learn about remote programming.

**Link(s) to work**

----------------------------------------------------------------------
### Day 46: Feb 27th, Monday

**Today's Progress**: making good progress converting my portfolio to React. Just need to create a footer and convert the projects into components

** Thoughts** 
- Could also try to use react routing to allow the user to sort projects by date or by technology.
- Need to write more stuff in the about section, add skills to the about sectiona and write an intro to the projects section.

**Link(s) to work** [React Profile Page](https://samwcoding.github.io/reactPortfolio/)

----------------------------------------------------------------------
### Day 47: Feb 28th, Tuesday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day : March 1st, Wednesday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day : March 2nd, Thursday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day : March 3rd, Friday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day : March 4th, Saturday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day : March 5th, Sunday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day : March 6th, Monday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day : March 7th, Tuesday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day : March 8th, Wednesday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

-----------------------------------------------------------------------
### Day : March 9th, Thursday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day : March 10th, Friday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day : March 11th, Saturday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day : March 12th, Sunday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day : March 13th, Monday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day : March 14th, Tuesday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day : March 15th, Wednesday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

-----------------------------------------------------------------------
### Day : March 16th, Thursday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day : March 17th, Friday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day : March 18th, Saturday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day : March 19th, Sunday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day : March 20th, Monday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day : March 21st, Tuesday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day : March 22nd, Wednesday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

-----------------------------------------------------------------------
### Day : March 23rd, Thursday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day : March 24th, Friday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day : March 25th, Saturday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day : March 26th, Sunday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day : March 27th, Monday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day : March 28th, Tuesday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day : March 29th, Wednesday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

-----------------------------------------------------------------------
### Day : March 30th, Thursday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day : March 31st, Friday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

----------------------------------------------------------------------
### Day : April 1st, Saturday

**Today's Progress**:

** Thoughts**

**Link(s) to work**

